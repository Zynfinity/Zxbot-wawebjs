  ...
  async func(m, conn, zx) {
    ...
    m.reply = async(text, option) => {
      if(typeof text == 'string'){
        conn.sendMessage(m.from, text, {...option, quotedMessageId: m.msgId, extra: {...ctwa}})
      }else{
    ...
    conn.sendList = async(from, title, txt, row, buttontext, option) => {
      ...
      await conn.sendMessage(from, list, {...option, extra: {...ctwa}})
    }
    conn.sendFileFromUrl = async (from, url, option, option1) => {
      ...
      send = await conn.sendMessage(from, media, { ...option, extra: {...ctwa}});
      return send
    };
    conn.sendFileFromBuffer = async (from, buffer, option) => {
      ...
      const send = await conn.sendMessage(from, media, { ...option, extra: {...ctwa}});
      return send
    };
    ...
    conn.sendSticker = async(anu, sticker, pack, author, option) => {
        anu.sendMessage(sticker, {...option, sendMediaAsSticker: true, stickerName: pack, stickerAuthor: author, extra: {...ctwa}})
    }
    conn.sendStickerFromUrl = async(from, url, pack, author, option) => {
      const media = await MessageMedia.fromUrl(url, {unsafeMime: true});
      conn.sendMessage(from, media, {...option, sendMediaAsSticker: true, stickerName: pack, stickerAuthor: author, extra: {...ctwa}})
  }
  },
};
