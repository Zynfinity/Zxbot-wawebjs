...
exports.y1s = async(type, url, quality) => {
    ...
    if(data.mess.includes('Invalid')) return({
        creator: '@ihsanafajar',
        ...data,
        status: false
    })
...
exports.expandurl = async(url) => {
    const gettoken = await axios.get('https://bitly.toolpie.com/')
    const $$ = cheerio.load(gettoken.data)
    const token = $$('#queryForm > input[type=hidden]').attr('value')
    const {data} = await axios.request('https://bitly.toolpie.com/', {
        method: 'post',
        data: new URLSearchParams(Object.entries({url: url, _token: token})),
        headers: {
            ...
            'cookie': `__gads=ID=c3af581f2ba691fb-22592eff7cd20030:T=1651185259:RT=1651185259:S=ALNI_MZJMDFr3U6ftWLphMK-nX6qDZFQPw; __atssc=google;1; _ga=GA1.2.1406110656.1651185264; _gid=GA1.2.1212470476.1651185264; ${gettoken.headers['set-cookie'][0]}; ${gettoken.headers['set-cookie'][1]} __atuvc=4|17; __atuvs=626b1670bdd8907c003`
        }
    })
    const $ = cheerio.load(data)
    expan = $('body').find('div.p-3.mt-2.bg-success.text-white > h6 > span').text()
    return(expan ? {status: true, result: expan} : {status: false})
}
...
exports.tiktokapi = async(url) => {
    ...
    const expandurll = await expandurl(url)
    if(expandurll.status){
        const {data} = await axios.get(`https://www.tiktok.com/node/share/video/${expandurll.result.split('/')[3]}/${expandurll.result.split('/')[5]}`, {
            headers:{
                'user-agent': 'Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Safari/537.36'
            }
        })
...
exports.anonUpload = async (buffer, nama) => {
    return new Promise((resolve, reject) => {
        ...
        fromBuffer(buffer).then((cek_file) => {
            fs.writeFileSync(`./lib/media/${nama}.${cek_file.ext}`, buffer)
            const bodyForm = new FormData()
            bodyForm.append('file', fs.createReadStream(`./lib/media/${nama}.${cek_file.ext}`))
        ...
        if (ytIdRegex.test(link)) {
            ...
            axios('https://www.y2mate.com/mates/id276/analyze/ajax', {
                ...
                .then(({
                    ...
                    let size = type == 'mp4' ? (asize.mp4.find(sz => sz.quality == quality + 'p')).size : $('#audio > table > tbody > tr:nth-child(1) > td:nth-child(2)').text()
...
exports.mediafire = async (url) => {
    ...
    return (media.status == 200 ? {
        ...
        return({status: false, message: 'Cek your link!'})
    }
}
