...
const { temp, sleep } = require('./tools')
...
exports.fetchPost = async (url) => {
  return new Promise(async (resolve, reject) => {
      urls = url.split('/')
    let { data } = await axios(url.split(url.includes('stories') ? urls[6] : urls[5])[0] + '?__a=1', {
      method: 'GET',
      headers: {
...
exports.mdl = async(query) => {
    return new Promise((resolve, reject) => {
        axios.get(`https://mydramalist.com/search?q=${query}&adv=titles&ty=68,77,83&so=relevance`).then(({data}) => {
            //console.log(data)
          const $ = cheerio.load(data)
          const result = []
...
exports.spotifysearch = async(query) => {
  data = await client.search({q: query, type:'track', limit: 1})
...
exports.twitterdl = async (url) => {
  return new Promise(async(resolve, reject) => {
      try{
          ...
          result.username != '' ? resolve({creator: '@ihsanafajar',status: true,data: result}) : resolve({creator: '@ihsanafajar',status: false})
      }catch{
          resolve({
              creator: '@ihsanafajar',
              status: false
...
exports.emojiMixSelector = (emoji1, emoji2) => {
  return new Promise(async(resolve, reject) => {
  await fetch('https://tenor.googleapis.com/v2/featured?key=AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q='+ encodeURIComponent(emoji1) +'_'+ encodeURIComponent(emoji2))
  .then(res => res.json())
  .then(json => {
   // console.log(json)
    if (json.results.length === 0) return resolve(undefined)
    resolve(json.results[0].url)
