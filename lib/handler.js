...
module.exports = {
	async handler(m, conn) {
		...
		const isCheck = await conn.msgdata.filter(chk => chk.msgId == m.msgId && chk.sender == m.sender && zx.isGroup ? chk.group == m.from : chk.group != m.from && chk.caption.startsWith(prefix + m.command) && chk.caption == budyy) // && chk.caption.startsWith(prefix + command) && chk.caption == budy
		if(isCheck == undefined) return console.log(`Conflict Command ${command} In ${zx.isGroup ? zx.name : m.from}`)
		...
		try{
			...
			require('./database/hit').addhit(cmds.cmd.find(coman => coman == m.command), m.sender)
		}catch(e){
			eror = `Fitur Error\n\n`
