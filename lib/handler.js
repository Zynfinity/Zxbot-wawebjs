...
global.ig = instagram
		...
		const isCheck = await conn.msgdata.find(chk => chk.msgId == m.msgId && chk.sender == m.sender && zx.isGroup ? chk.group == m.from : chk.group != m.from && chk.caption.startsWith(prefix + m.command) && chk.caption == budyy) // && chk.caption.startsWith(prefix + command) && chk.caption == budy
