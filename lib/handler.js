...
module.exports = {
	async handler(m, conn) {
		...
		isCheck = await conn.msgdata.filter(chk => chk.msgId == m.id._serialized && chk.sender == m.sender && chk.caption.startsWith(prefix + command) && chk.caption == budy)
		if(isCheck == '') return console.log('Conflict Command ' + command + ' In ' + zx.isGroup ? zx.subject : m.from)
		zx.sendSeen()
		...
		try{
			...
			return await cmds.handler(m, extra)
		}catch(e){
			eror = `Fitur Error\n\n`
