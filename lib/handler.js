...
module.exports = {
	async handler(m, conn) {
		...
		cmdstick = type == 'sticker' ? data.stcmd.find(stc => stc.id == m._data.filehash) : false
		...
		if(command != undefined && command != '' && config.antispam && !isOwner){
			spam = await Object.values(global.commands).find(sp => !sp.disabled && sp.cmd && sp.cmd.includes(command))
		...
		if((m.command == 'unmute' || m.command == 'mute') && data.mute.includes(m.from) && (isOwner || isAdmin)){
			return await require('../commands/mute.js').handler(m, conn)
		}
		if(data.mute.includes(m.from) && !isOwner) return
		//exec command
		//console.log(m.id)
		//if(m.isBot) return
		cmds = await Object.values(global.commands).find((rescmd) => !rescmd.disabled && rescmd.cmd.includes(command))
		//console.log(cmds)
		...
		if(!isOwner && m.from in conn.cooldown){
			sisa = await ms(conn.cooldown[m.from].timestamp - Date.now())
			return zx.sendMessage(`_Grup ini sedang dalam masa cooldown_\n_Silahkan tunggu *${sisa.seconds},${sisa.milliseconds}* detik_\n\n_Note : Untuk mematikan ini, anda harus menonaktifkan mode game dengan .game off_`, {quotedMessageId: m.id._serialized})
		}
		else if(!isOwner && m.sender in conn.cooldown){
			sisa = await ms(conn.cooldown[m.sender].timestamp - Date.now())
			return zx.sendMessage(`_command sedang cooldown.._\n_Silahkan tunggu *${sisa.seconds},${sisa.milliseconds}* detik_`, {quotedMessageId: m.id._serialized})
		}
		zx.sendSeen()
		...
		try{
			...
			if(cmds.game && zx.isGroup){
				games = await db.collection('groups').findOne({id: m.from})
				if(games == null) return sfail(m, 'game')
				if(!games.game) return sfail(m, 'game')
