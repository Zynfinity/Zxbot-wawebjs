...
module.exports = {
    ...
    async handler(m, {conn, msgId, zx, text, command, hasQuotedMsg, hasMedia, quotedMsg}){
        try{
            if(!text) return await conn.reply(m, `Masukkan teksnya dengan format text1|teks2\nExample : .${command} test|oke`, msgId)
            ...
            else conn.reply(m, 'reply gambar/sticker dengan caption .' + command, msgId)
        }catch(e){
