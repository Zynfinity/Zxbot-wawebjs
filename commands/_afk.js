...
module.exports = {
  function: true,
  async handler(m, { conn, quotedMsg, mentionedJidList, command }) {
    if(command == 'afk') return
    afkdata = await data.findOne({ id: m.sender.id });
    if (afkdata != null) {
      ...
      data.deleteOne({ id: m.sender.id }).then((s) => m.reply(eafk));
    }
    if (quotedMsg) {
      afkdata = await data.findOne({ id: quotedMsg.sender.id });
      if (afkdata != null) {
      ...
      mentionedJidList.map(async (res) => {
