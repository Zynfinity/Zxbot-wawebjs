module.exports = {
    ...
    async handler(m, {conn, zx, args}){
        if(args[0] == 'open'){
            if(!zx.groupMetadata.announce) return await conn.reply(m, 'Group sudah dibuka sebelumnya')
            await zx.setMessagesAdminsOnly(false)
            //conn.reply(m, 'Berhasil membuka group!\nsekarang semua member bisa mengirim pesan!')
        }
        else if(args[0] == 'close'){
            if(zx.groupMetadata.announce) return await conn.reply(m, 'Group sudah ditutup sebelumnya')
