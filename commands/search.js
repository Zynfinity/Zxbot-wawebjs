    ...
    async handler(m, {conn, msgId, text, args}){
        try{
            ...
            if(m.command == 'wattpad'){
                ...
                await conn.sendFileFromUrl(m.from, getdata[0].thumb, {ctwa: {type: 'link'}, caption: await tools.parseResult('WATTPAD SEARCH', getdata, {delete: ['thumb']}), quotedMessageId: msgId})
            }
            else if(m.command == 'happymod'){
                getdata = await happymod(text)
                if(getdata.data == '') return m.reply('apk tidak ditemukan!')
                await conn.sendFileFromUrl(m.from, getdata.data[0].thumb, {ctwa: {type: 'link'}, caption: await tools.parseResult('HAPPYMOD SEARCH', getdata.data, {delete: ['thumb']}), quotedMessageId: msgId})
            }
            else if(m.command == 'mangatoon'){
                getdata = await mangatoon(text)
                if(getdata == '') return m.reply('Manga tidak ditemukan!')
                await conn.sendFileFromUrl(m.from, getdata[0].comic_thumb, {ctwa: {type: 'link'}, caption: await tools.parseResult('MANGATOON SEARCH', getdata, {delete: ['status', 'creator', 'comic_thumb']}), quotedMessageId: msgId})
            }
