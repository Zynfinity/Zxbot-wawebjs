...
module.exports = {
    ...
    async handler(m, {conn, args}){
        if (!args[0]) return m.reply("masukkan nomor suratnya!");
        if (isNaN(args[0])) return m.reply("Input harus berupa nomor!");
        if (args[0] > 114) return m.reply("surat al-quran hanya berjumlah 114!");
